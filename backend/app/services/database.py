...
class DatabaseService:
    ...
    async def list_stories(
        ...
        videos_result, records_result = await asyncio.gather(
            asyncio.to_thread(self.client.table("video_assets").select("*").in_(
                "story_id", story_ids
            ).order("created_at", desc=True).execute),
            asyncio.to_thread(self.client.table("publish_records").select("*").in_(
                "story_id", story_ids
            ).order("created_at", desc=True).execute)
        )
    ...
    async def get_content_stats(self) -> ContentStats:
        ...
        # Run all supabase queries concurrently in threads to prevent blocking the event loop
        # and to speed up the endpoint by running I/O in parallel.
