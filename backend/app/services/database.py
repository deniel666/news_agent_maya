...
class DatabaseService:
    ...
    async def get_dashboard_stats(self) -> dict:
        ...
        # ⚡ Bolt Optimization: Batch concurrent database stats queries
        briefings, videos, posts = await asyncio.gather(
            asyncio.to_thread(
                self.client.table("weekly_briefings").select(
                    "id, status", count="exact"
                ).execute
            ),
            asyncio.to_thread(
                self.client.table("weekly_videos").select(
                    "id", count="exact"
                ).execute
            ),
            asyncio.to_thread(
                self.client.table("social_posts").select(
                    "id", count="exact"
                ).execute
            )
        )
    ...
    async def get_content_stats(self) -> ContentStats:
        ...
        # ⚡ Bolt Optimization: Batch 5 concurrent queries
