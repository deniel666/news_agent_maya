...
class DatabaseService:
    ...
    async def get_dashboard_stats(self) -> dict:
        ...
        # BOLT OPTIMIZATION:
        # Replaced 3 sequential blocking Supabase .execute() calls with
        # asyncio.to_thread and asyncio.gather to execute queries concurrently,
        # reducing latency and avoiding blocking the main event loop.
        briefings_task = asyncio.to_thread(
            self.client.table("weekly_briefings").select("id, status", count="exact").execute
        )
        videos_task = asyncio.to_thread(
            self.client.table("weekly_videos").select("id", count="exact").execute
        )
        posts_task = asyncio.to_thread(
            self.client.table("social_posts").select("id", count="exact").execute
        )

        briefings, videos, posts = await asyncio.gather(
            briefings_task, videos_task, posts_task
        )
    ...
    async def get_content_stats(self) -> ContentStats:
        ...
        # BOLT OPTIMIZATION:
        # Executing 5 independent database aggregations concurrently
        # using asyncio.to_thread and gather. Reduces latency significantly.
        stories_task = asyncio.to_thread(
            self.client.table("stories").select("status, story_type", count="exact").execute
        )
        videos_task = asyncio.to_thread(
            self.client.table("video_assets").select("language", count="exact").execute
        )
        publishes_task = asyncio.to_thread(
            self.client.table("publish_records").select("platform, status", count="exact").execute
        )
        this_week_task = asyncio.to_thread(
            self.client.table("stories").select("id", count="exact").gte("created_at", week_ago).execute
        )
        this_month_task = asyncio.to_thread(
            self.client.table("stories").select("id", count="exact").gte("created_at", month_ago).execute
        )
