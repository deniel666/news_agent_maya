...
class DatabaseService:
    ...
    async def get_dashboard_stats(self) -> dict:
        ...
        # Run queries concurrently in thread pool
    ...
    async def list_stories(
        ...
        # Batch fetch related assets concurrently
        videos_result, records_result = await asyncio.gather(
            asyncio.to_thread(
                self.client.table("video_assets").select("*").in_(
                    "story_id", story_ids
                ).order("created_at", desc=True).execute
            ),
            asyncio.to_thread(
                self.client.table("publish_records").select("*").in_(
                    "story_id", story_ids
                ).order("created_at", desc=True).execute
            )
        )
