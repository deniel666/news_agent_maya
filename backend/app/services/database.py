...
class DatabaseService:
    ...
    async def get_dashboard_stats(self) -> dict:
        ...
        # ⚡ Bolt Optimization: Run independent synchronous Supabase queries concurrently
        # wrapped in asyncio.to_thread to prevent I/O blocking
        ...
        briefings, videos, posts = await asyncio.gather(briefings_task, videos_task, posts_task)
    ...
    async def get_content_stats(self) -> ContentStats:
        # Get recent counts limits
        now = datetime.utcnow()
        week_ago = (now - timedelta(days=7)).isoformat()
        month_ago = (now - timedelta(days=30)).isoformat()

        # ⚡ Bolt Optimization: Run independent synchronous Supabase queries concurrently
        # wrapped in asyncio.to_thread to prevent sequential I/O blocking
