...
class DatabaseService:
    ...
    async def get_dashboard_stats(self) -> dict:
        ...
        briefings_query = self.client.table("weekly_briefings").select(
            "id, status", count="exact"
        )

        videos_query = self.client.table("weekly_videos").select(
            "id", count="exact"
        )

        posts_query = self.client.table("social_posts").select(
            "id", count="exact"
        )
