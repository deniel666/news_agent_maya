...
class DatabaseService:
    ...
    async def get_dashboard_stats(self) -> dict:
        ...
        # ⚡ Bolt Optimization: Batch concurrent I/O operations
        # Replaced 3 sequential blocking DB calls with threaded execution via asyncio.gather
        # Reduces cumulative network wait time from O(n) to O(1)
        briefings_task = asyncio.to_thread(self.client.table("weekly_briefings").select("id, status", count="exact").execute)
        videos_task = asyncio.to_thread(self.client.table("weekly_videos").select("id", count="exact").execute)
        posts_task = asyncio.to_thread(self.client.table("social_posts").select("id", count="exact").execute)
    ...
    async def get_content_stats(self) -> ContentStats:
        ...
        # ⚡ Bolt Optimization: Batch concurrent I/O operations
        # Replaced 5 sequential blocking DB calls with threaded execution via asyncio.gather
        # Significantly reduces query latency for dashboard loads
