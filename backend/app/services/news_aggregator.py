...
class NewsAggregatorService:
    ...
    def _process_rss_content(self, content: str, source_name: str, feed_url: str, cutoff: datetime) -> List[NewsArticle]:
        """Process RSS content synchronously (CPU-bound)."""
        articles = []
        try:
            ...
            print(f"Error parsing RSS content for {source_name}: {e}")
        return articles

    def _process_nitter_content(self, content: str, username: str, cutoff: datetime) -> List[NewsArticle]:
        """Process Nitter content synchronously (CPU-bound)."""
        articles = []
        try:
            feed = feedparser.parse(content)
            ...
            print(f"Error parsing Nitter content for {username}: {e}")
        return articles
            ...
            async with session.get(feed_url) as response:
                if response.status == 200:
                    ...
                    return await asyncio.to_thread(
                        self._process_rss_content, content, source_name, feed_url, cutoff
                    )
        except Exception as e:
