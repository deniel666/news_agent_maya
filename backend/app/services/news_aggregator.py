...
class NewsAggregatorService:
    ...
    def _parse_rss_feed_blocking(self, content: str, source_name: str, cutoff: datetime) -> List[NewsArticle]:
        """Parse RSS feed content in a thread to avoid blocking loop."""
    ...
    def _parse_nitter_feed_blocking(self, content: str, username: str, cutoff: datetime) -> List[NewsArticle]:
        """Parse Nitter feed content in a thread."""
        feed = feedparser.parse(content)
        articles = []
            ...
            try:
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        return await loop.run_in_executor(
                            None,
                            self._parse_nitter_feed_blocking,
                            content,
                            username,
                            cutoff
                        )
