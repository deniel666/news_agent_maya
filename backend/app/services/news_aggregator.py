...
def parse_rss_feed_content(content: str, source_name: str, cutoff: datetime) -> List[NewsArticle]:
    articles = []
    try:
        feed = feedparser.parse(content)
        for entry in feed.entries[:30]:
            published = parse_date_string(entry.get("published"))
...
def parse_twitter_feed_content(content: str, username: str, cutoff: datetime) -> List[NewsArticle]:
    articles = []
    try:
        feed = feedparser.parse(content)
        for entry in feed.entries[:20]:
            ...
            content_clean = clean_html_text(entry.get("title", ""))
